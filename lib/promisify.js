 ...
 */
var getParameterNames = function getParameterNames(signature) {
  var COMMENTS = /((\/\/.*$)|(\/\*[\s\S]*?\*\/))/mg;
  ...
  var code = signature.replace(SPACES, '').replace(COMMENTS, '').replace(FAT_ARROWS, '').replace(DEFAULT_PARAMS, '').replace(BEFORE_OPENING_PAREN, '').replace(AFTER_CLOSING_PAREN, '$1');
 ...
 */
var parseSignature = function parseSignature(signature) {
  var paramNames = getParameterNames(signature);
