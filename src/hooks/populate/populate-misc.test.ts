...
function schemaFcn(_hook: any, options: any) {
  fcnOptions = options;
...
function user(app: any) {
  ...
  app.service('users').hooks({
    before: {
      all: [
        (hook: any) => {
          userHookFlag1 = hook.params.userHookFlag1;
        },
      ],
    },
  });
...
function team(app: any) {
  ...
  app.service('teams').hooks({
    after: {
      all: [
        (hook: any) => {
          teamHookFlag1 = hook.params.teamHookFlag1;
        },
...
describe('services populate - hook.params passed to includes', () => {
  ...
  it('hook.params passed to includes', () => {
    ...
    return teams.find({ query: { id: 0 }, userHookFlag1: 'userHookFlag1' }).then((result: any) => {
  ...
  it('ignores pagination by default', () => {
    whichSchema = 'schemaDefault';
    return teams.find({ query: { id: 0 } }).then((result: any) => {
      assert.deepEqual(result, resultDefault);
    });
  });
...
describe('services populate - schema may be a function', () => {
  ...
  it('function gets hook and options', () => {
    whichSchema = 'schemaDefaultFcn';
    return teams
      .find({ query: { id: 0 }, userHookFlag1: 'userHookFlag1', teamHookFlag1: 'teamHookFlag1' })
      .then((result: any) => {
