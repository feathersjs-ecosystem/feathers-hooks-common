...
async function ex1 () {
  ...
  const context: any = { method: 'find', result: await posts.find({ query: { id: 1 } }), params: {} };
...
async function ex3 () {
  ...
  return fastJoin((_context: any) => postResolvers, (_context: any) => query)(context);
}
      ...
      comments: {
        resolver: ($select: any, $limit: any, $sort: any) => async (post: any) => {
          post.comments = await comments.find({
            query: {
              postId: post.id, $select, $limit: $limit || 5, [$sort]: { createdAt: -1 }
            }
          });
...
async function ex9 () {
  ...
  const postResolvers = {
    ...
    joins: {
      ...
      reputation_author: (..._args: any[]): any => async (post: any, context: any): Promise<any> => {
      ...
      comments: {
        resolver: (..._args: any[]) => async (post: any, context: any) => {
  ...
  const context: any = { method: 'find', result: await posts.find({ query: { id: { $in: [1, 2, 3, 4] } } }), params: {} };
