...
describe('common hook discard', () => {
  describe('removes fields', () => {
    ...
    const afterBoth =   (): any => ({ type: 'after',  result: [{ first: 'John',    last: 'Doe' }, { first: 'Jane', last: 'Doe' }] });
    const afterPage =   (): any => ({ type: 'after',  result: { total: 2, skip: 0, data: [{ first: 'John', last: 'Doe' }, { first: 'Jane', last: 'Doe' }] } });
  ...
  describe('handles dot notation', () => {
    ...
    const ctx  = (): any => ({ type: 'before', method: 'create', data:   { empl: { name: { first: 'John', last: 'Doe' }, status: 'AA' }, dept: 'Acct' } });
    const ctx2 = (): any => ({ type: 'after',  method: 'get',    result: { property: null, foo: 'bar' } });
